# Babylon.js QA And Release Checklists

Use this before calling a Babylon.js browser game complete, premium, release-ready, or fixed.
...
- Physics-heavy games: engine choice (Havok/Cannon.js), fixed timestep, body count, collision/trigger path, high-speed tunneling check, and restart body cleanup verified.
...
- Use the visual scorecard from `babylon-aaa-graphics-builder/references/visual-scorecard.md`.
...
- For generated 3D assets, confirm imported models have correct scale, orientation, material readability, collision proxies, and animation groups in active gameplay.
...
When draw calls, asset counts, materials, or post-processing changed:

- Record active meshes, materials, textures, draw calls.
- Record FPS/frame time via `engine.getFps()`.
- Record physics engine, timestep, body count when physics changed.
...
- Babylon Inspector, debugLayer, debug UI, diagnostics overlays, verbose logs, and test shortcuts are gated or removed from player-facing release.
...
- Browser support assumptions documented (WebGL 2.0 required for Babylon.js 7+).
...
- Babylon Inspector visible to players.
...
- Babylon.js `scene.dispose()` not called on scene transition, leaking resources.
