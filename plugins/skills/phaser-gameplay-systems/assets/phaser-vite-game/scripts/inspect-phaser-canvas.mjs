 ...
 * Opens a Phaser game URL in Playwright, captures a screenshot,
...
try {
  ...
  // Check that the PNG has meaningful pixel data
  // PNG signature + IHDR chunk means > 100 bytes
  if (screenshot.length < 1024) {
    console.error('FAIL: screenshot likely blank or too small');
    process.exit(1);
  }

  // Simple nonblank check: at least some non-background pixels exist
  // We check the PNG has IHDR and IDAT chunks
  ...
  console.log(`PASS: canvas screenshot captured (${screenshot.length} bytes, ${args.mobile ? 'mobile' : 'desktop'})`);
} catch (err) {
