---
name: babylon-gameplay-systems
description: "Build and iterate playable Babylon.js game systems. Combines starter scaffold creation, architecture, game design, level design, gameplay implementation, combat/encounter design, and game-feel tuning (hitstop, screenshake, easing, impact feedback). Use for first playable slices, new Vite/TypeScript/Babylon.js game setup, design briefs, core loops, level/arena/track/wave/hole/puzzle design, game loops, entity systems, input, collision/physics (Havok/Cannon.js), scoring, objectives, audio hooks, camera, controls, difficulty, feedback, juice, and maintainable structure."
---

# Babylon.js Gameplay Systems

## Purpose

Create or evolve a playable browser game loop with clear Engine/Scene ownership, responsive controls, deterministic update order, strong design intent, playable spaces, and verified player-facing behavior.
...
```bash
python3 <this-skill-dir>/scripts/create_babylon_game.py ./my-game
```

The script copies `assets/babylon-vite-game/`, rewrites the project name in `package.json`, and keeps generated games self-contained with their own visual test and canvas-inspection script. Use `--force` only when the target directory may be overwritten.

## Library Guidance

- Use TypeScript, Vite, `@babylonjs/core` modules.
- Physics/collision engine choice (custom collision vs Havok vs Cannon.js), timestep, and impostor strategy: follow `references/physics-engine-selection.md`.
- Default camera: `BABYLON.ArcRotateCamera` for orbit/third-person; `BABYLON.UniversalCamera` for FPS.
- Default lights: `BABYLON.HemisphericLight` (ambient/ground fill) + `BABYLON.DirectionalLight` (key light with `ShadowGenerator`).
...
- Using multiple Engine instances instead of one. A single `BABYLON.Engine` serves all scenes.
- Forgetting `scene.dispose()` on restart/transition, leaking meshes, materials, textures, and event listeners.
- Havok WASM not loaded before creating physics bodies.
- Not calling `engine.resize()` on window resize, causing stretched or clipped rendering.
