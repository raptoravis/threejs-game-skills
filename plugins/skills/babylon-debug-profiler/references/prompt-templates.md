# Babylon.js Debug/Profile Prompt Templates
...
Use `babylon-debug-profiler` for mobile render/input bugs, or `threejs-game-ui-designer` when the main issue is touch-control/HUD layout.
...
Use `babylon-debug-profiler` to profile and improve this Babylon.js game.
...
Measure:
- frame time (engine.getFps / engine.getDeltaTime)
...
Use `babylon-debug-profiler` to diagnose this Babylon.js issue:
...
- Check canvas size, engine size, camera type (ArcRotateCamera/UniversalCamera), alpha/beta/radius, near/far planes, lights, material visibility, asset paths, async loading, and runRenderLoop ownership.
